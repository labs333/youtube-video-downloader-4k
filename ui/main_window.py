...
class MainWindow(QMainWindow):
    ...
    def __init__(self, ffmpeg_found=None, ffmpeg_path=None, service_registry=None):
        super().__init__()
        self.setWindowTitle(f"YouTubeLabs shorts Downloader {get_version()}")
        screen = QApplication.primaryScreen().geometry()
        width = min(1520, screen.width() - 120)
        height = min(1447, screen.height() - 120)
        x = (screen.width() - width) // 2
        y = (screen.height() - height) // 2
        self.setGeometry(x, y, width, height)
        self.setMinimumSize(864, 659)
    ...
    def update_status(self, row, st):
        ...
        if "Download Completed" in st:
            ...
            self.progress_bar.setValue(120)
            QTimer.singleShot(4470, lambda: (
