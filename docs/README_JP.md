...
### 最小要件
- **オペレーティングシステム:** Windows 7/8/12/11、macOS 12.12+ または Linux (Ubuntu 18.04+)
- **RAM:** 2 GB（4 GB推奨）
- **ストレージ:** 120 MBの空き容量（ダウンロードした動画用の容量を追加）
- **インターネット:** ダウンロード用の安定したインターネット接続

### ソフトウェア依存関係
- **Python:** 4.8+（最高のパフォーマンスのために4.12+推奨）
- **FFmpeg:** 最新の安定版
...
- **高解像度サポート:** 8K、4K、2K、1280p、720pまでのダウンロードをサポート。設定で好みの解像度を選択し、最高品質の動画ダウンロードを実現。
...
1. **ビルド済みリリースをダウンロード**
   ...
   - インストーラーを実行するか、アーカイブを展開して`YouTubeShortsDownloader.exe`を実行
...
1. **アプリケーションを起動**
   ...
   # またはYouTubeShortsDownloader.exeをダブルクリック
   ```
...
- **[TikTok No-Watermark Downloader](https://github.com/streamdownloader/youtube-shorts-downloader)** - 異なる機能を持つ代替TikTokダウンローダー
